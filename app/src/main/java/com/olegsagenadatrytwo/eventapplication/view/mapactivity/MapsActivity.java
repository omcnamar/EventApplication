package com.olegsagenadatrytwo.eventapplication.view.mapactivity;
...
public class MapsActivity extends FragmentActivity implements OnMapReadyCallback {
    ...
    public void onMapReady(GoogleMap googleMap) {
        ...
        List<Event> events = singleTonEventsList.getEvents();
        for (Event event : events) {
            latLng = new LatLng(Double.parseDouble(event.getVenue().getLatitude()), Double.parseDouble(event.getVenue().getLongitude()));
            mMap.addMarker(new MarkerOptions().position(latLng).title(event.getName().getText()));
        }
        ...
        String lat = singleTonMyLocation.getLat();
        String lon = singleTonMyLocation.getLon();
